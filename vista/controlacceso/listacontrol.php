...
<body class="fondo  <?php echo ($_SESSION['id_rol'] === 3) ? 'agente' : ''; ?>" id="listacontrol-vista">
    ...
    <div class="container mt-3 border border-2 p-3" style="border-radius: 15px; max-height: 150px; max-width: 50%; background: linear-gradient(180deg,rgb(185, 178, 178) 70%, #878c8d 100%);">
        <?php if ($_SESSION['id_rol'] === 3) { // Si no es agente, mostrar "Generar Reporte" 
        ?>
            <div class="col-md-6">
                ...
                <a href="agregaracceso.php" class="boton d-flex justify-content-center align-items-center mx-auto mt-3"
        ...
        ?>
            <div class="col-md-6">
                ...
                <a class="boton d-flex justify-content-center align-items-center mx-auto mt-3" href="../../dompdf/reporte_pdf_acceso.php"
    ...
    <script>
        function eliminar() {
            var respuesta = confirm("¿Desea eliminar el acceso?")
            return respuesta
        }
    </script>
    <?php
    ...
    <div class="container mt-3">
        <div class="estilo-tabla">
            <table>
                <thead>
                    <tr>
                        <?php if ($_SESSION['id_rol'] != 3) { // Mostrar encabezados completos si no es agente 
                        ?>
                            <th>ID Acceso</th>
                            <th>ID Empleado</th>
                            <th>Fecha</th>
                ...
                <tbody>
                    ...
                    if ($_SESSION['id_rol'] == 3) {
                        ...
                        $sql = $conexion->query("SELECT Fecha, Hora_Entrada, Hora_Salida, Observacion FROM control_acceso WHERE Id_Empleado = $idempleado");
                    } else {
                        // Consulta completa para otros roles
                        $sql = $conexion->query("SELECT * FROM control_acceso");
                    }

                    while ($datos = $sql->fetch_object()) { ?>
                        <tr>
                            <?php if ($_SESSION['id_rol'] != 3) { ?>
                                <td><?= $datos->ID_Control ?></td>
                                ...
                                <!-- <td>
                                    <div class="botones-acciones">
                                        <a href="modificarcontrolacceso.php?id=<?= $datos->ID_Control ?>" class="botoneditar">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </a>
                                        <a onclick="return eliminar()" href="listacontrol.php?id=<?= $datos->ID_Control ?>" class="botoneliminar">
