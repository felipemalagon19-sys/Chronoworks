...
<body class="fondo <?php echo ($_SESSION['id_rol'] === 3) ? 'agente' : ''; ?>">
    ...
    <?php if ($_SESSION['id_rol'] != 3): ?>
        <div class="container mt-3 border border-2 p-3" style="border-radius: 15px; max-height: 150px; max-width: 50%;   background: linear-gradient(180deg,rgb(185, 178, 178) 70%, #878c8d 100%);">
            <div class="col-md-6">
                ...
                <a href="agregarroles.php" class="boton d-flex justify-content-center align-items-center mx-auto mt-3"
    ...
    <script>
        function eliminar() {
            var respuesta = confirm("¿Desea eliminar el rol?")
            return respuesta
        }
    </script>
    <?php
        ...
        <div class="estilo-tabla">
            <table>
                ...
                <tbody>
                    ...
                    if ($_SESSION['id_rol'] == 3) {
                        // Para agentes, mostrar solo su propio rol
                        $sql = $conexion->query("SELECT nombre FROM roles WHERE ID_Rol = " . $_SESSION['id_rol']);
                    } else {
                        // Para Admin y Líder, mostrar todos los roles
